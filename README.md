...
- **Fast image transfer** — locally, raw RGB frames go to the terminal through POSIX shared memory (kitty graphics `t=s`): no PNG encode or decode anywhere, only the shm name crosses the tty, and the terminal unlinks the object after reading. `general.shm_transfer: false` falls back to PNG temp files (`t=t`) — chosen automatically inside zellij, which rejects shared memory; over SSH, pages stream as base64 PNG (10–40× smaller than raw for text pages).
...
- **Page grid** (`g`) — a thumbnail overview of the whole book: adaptive columns, page numbers, smooth per-cell scrolling with edge thumbs clipped mid-thumb. `h`/`j`/`k`/`l` move the selection (eased scroll follows), wheel pans without touching the selection, `Ctrl+D`/`Ctrl+U` jump a screenful, `Enter` or click opens a page. `i`/`o` (or mod+wheel) resize the thumbnails, and `z`/`t` re-render them inverted/auto-cropped. Thumbnails render on the background worker and stream in as they finish — the grid opens instantly and stays responsive even on image-heavy books — going out at grid size through the shared-memory raw-RGB path with no PNG work.
...
Requirements: Zig `0.16.0`, a terminal with the Kitty image protocol (Ghostty, kitty ≥ 0.19, WezTerm ≥ 20220319, Konsole ≥ 22.04; inside a multiplexer: zellij ≥ 0.45, or tmux ≥ 3.3 with `allow-passthrough on`), and on macOS the Xcode command-line tools. In a terminal without the protocol `re` exits with a message naming these.
